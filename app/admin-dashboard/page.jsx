...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      ...
      try {
        ...
        console.error('[QP Dashboard] Gagal memuat pengguna:', error);
  ...
  useEffect(() => {
    if (!state.tokoId) return undefined;
    setLoadingData(true);
    const base = db.collection('toko').doc(state.tokoId);
    const unsubs = [
      base.collection('pesanan').onSnapshot((snap) => {
        setOrders(snap.docs.map((doc) => ({ id: doc.id, ...doc.data() })));
      }, (error) => console.error('[QP Dashboard] Pesanan:', error)),
      base.collection('produk').onSnapshot((snap) => {
        setProducts(snap.docs.map((doc) => ({ id: doc.id, ...doc.data() })));
      }, (error) => console.error('[QP Dashboard] Produk:', error)),
      base.collection('chatPelanggan').onSnapshot((snap) => {
        setChats(snap.docs.map((doc) => ({ id: doc.id, ...doc.data() })));
      }, (error) => console.error('[QP Dashboard] Chat:', error)),
    ];
  ...
  if (state.loading) return <main className="admin-dashboard-page">
      <AdminNav active="dashboard" /><div className="admin-dashboard-empty">Memuat akses admin...</div></main>;
  if (!state.allowed) return <main className="admin-dashboard-page"><div className="admin-dashboard-denied"><h2>Dashboard Admin</h2><p>{state.error}</p></div></main>;
