...
export async function setProductStock(tokoId, productId, stock, uid) {
  const numericStock = Number(stock);
  if (!Number.isFinite(numericStock) || numericStock < 0) throw new Error('INVALID_STOCK');
  return productsRef(tokoId).doc(productId).set({
    stok: numericStock,
    updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
    updatedBy: uid,
    stokTerakhirBerubah: firebase.firestore.FieldValue.serverTimestamp(),
    stokSumber: 'admin-stok',
  }, { merge: true });
}
