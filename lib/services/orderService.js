...
export function subscribeCustomerOrders(uid, onData, onError) {
  if (!uid) return () => {};
  return db.collectionGroup('pesanan').where('uidPelanggan', '==', uid).onSnapshot(
    (snapshot) => onData?.(snapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
      tokoId: doc.ref.parent.parent?.id || doc.data()?.tokoId || null,
    }))),
    onError
  );
}
